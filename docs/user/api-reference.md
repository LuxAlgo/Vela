...
| `label:click` | `{ id, indicatorId }` | The user clicked one of an indicator's label drawings (`label.new(...)`, or a label a native indicator emits). `id` is the label's own id, `indicatorId` the owning indicator (the same id as `IndicatorHandle.id`). Hosts key interactive labels off it — a signal callout, a news marker, an alert badge. Never fires while a drawing tool, the measure ruler, or the eraser is armed: those own the click. |
...
| `onLabelClick(cb)` | Subscribe to clicks on indicator label drawings — the raw renderer seam behind the chart's `label:click` event, for host chrome that already listens on the renderer. Payload `{ indicatorId, labelId }`. Silent no-op unsubscribe on a renderer without indicator-drawing hit-testing. |
