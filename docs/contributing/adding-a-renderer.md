...
- **Label clicks** — the optional `onLabelClick(cb)` method: report a tap that lands on an indicator label drawing as `{ indicatorId, labelId }`; the core re-emits it as the `label:click` chart event. The native renderer resolves it against the per-frame hit-rect list its label painter already keeps for hover tooltips (`IndicatorDrawingSlices.labelAt`), so a renderer with its own drawing painter records one rect per painted label, tagged with the label and indicator ids, and answers both hover and click from that list. Detected by presence; a renderer without indicator-drawing hit-testing omits it and the event never fires.
